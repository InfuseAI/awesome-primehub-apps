# Creating Your Own PrimeHub App

This tutorial demostrates how to create your own PrimeHub app using `PhAppTemplate` CRD.

For more information about the PrimeHub app, please check our [design document](03-system-design.md).
...
Every PrimeHub app is created inside a group context. If you have [enabled group volume feature](https://docs.primehub.io/docs/guide_manual/admin-group#shared-volume) for that group, then that group volume will be mounted to your application container at `<group-volume>/phapplications/<app-id>`. You can use the predefined environment variable `PRIMEHUB_APP_ROOT` to tell your application to store persistant data into that location.
